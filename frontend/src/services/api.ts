...
export async function runLiveHarmonize(text: string, uom: string = 'EA'): Promise<LiveHarmonizeResult> {
  const response = await fetch(`${BASE_URL}/matching/compare-live`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text1: text, uom1: uom, text2: text, uom2: uom })
  });
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error(err.detail || 'Failed to extract attributes');
  }
  const data = await response.json();
  return {
    source_description: text,
    normalized_description: text.trim().toUpperCase(),
    source_uom: uom,
    normalized_uom: uom.trim().toUpperCase(),
    unspsc_code: data.unspsc_code || '40141607',
    category_name: data.category_name || 'Industrial Equipment & Valves',
    standardized_description: data.standardized_description || text,
    vector_dimension: 384,
    vector_sample: [],
    extracted_attributes: data.extracted_attributes || data.tokens1 || {
      noun: text.split(' ')[0] || 'MATERIAL',
      raw_text: text,
      uom: uom
    }
  };
...
export async function runLiveCompare(text1: string, text2: string, uom1: string = 'EA', uom2: string = 'EA'): Promise<LiveCompareResult> {
  const response = await fetch(`${BASE_URL}/matching/compare-live`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ text1, uom1, text2, uom2 })
  });
  if (!response.ok) {
    const err = await response.json().catch(() => ({}));
    throw new Error(err.detail || 'Failed to compare materials');
  }
